...
accuracy_cv = cross_val_score(dt_al, features_al, labels_al, cv=n_cv)

sec_dt_al_cv = (time.time() - time_dt_al_cv) # Time end CV

kf_cv_al = KFold(n_splits=n_cv, random_state=None, shuffle=False)
...
for train_index_al, test_index_al in kf_cv_al.split(features_al):
    ...
    scores_al_dt_mse.append(accuracy_r2)
    scores_al_dt_mae.append(accuracy_mse)
...
#%% R2 Força bruta
#mean_test_labels_al = round(np.mean(test_labels_al),2)
#y_mean = np.full((9608,1), mean_test_labels_al, dtype=float)
